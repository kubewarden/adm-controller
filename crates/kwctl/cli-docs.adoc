...
+
Possible values: `opa`, `gatekeeper`, `kubewarden`, `wasi`, `ferricel`
...
* `--compile-to-wasm <FILE>` — Compile the VAP CEL expressions into a standalone Wasm module and write it to FILE
* `-f`, `--force <FORCE>` — Overwrite FILE and metadata.yml if they already exist (only valid together with --compile-to-wasm)
